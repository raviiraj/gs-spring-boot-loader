...
```
$ docker build --build-arg JAR_FILE=build/libs/*.jar -t ravhitler/gs-spring-boot-docker .
```

This command builds an image and tags it as `ravhitler/gs-spring-boot-docker`.
...
2020-04-3 07:29:41.729  INFO 1 --- [           main] hello.Application                        : Starting Application on b94c86e91cf9 with PID 1 (/app started by spring in /)
...
```
...
```
$ ./gradlew jib --image=ravhitler/gs-spring-boot-docker
```
...
----
$ docker run -p 8080:8080 -t ravhitler
